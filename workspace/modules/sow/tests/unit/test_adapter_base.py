...
def test_one_terminal_per_subscription_by_default() -> None:
    ...
    g.register_subscription("sub-a", "anthropic")
    g.acquire("sub-a", "n1")
...
def test_raised_allowance_permits_more() -> None:
    g = SubscriptionGovernor()
    g.register_subscription("sub-a", "anthropic", allowance=2)  # OP-6 operator-ordered raise
    g.acquire("sub-a", "n1")
    g.acquire("sub-a", "n2")
...
def test_governor_hard_caps_allowance_at_two() -> None:
    ...
    with pytest.raises(ValueError, match="exceeds the governor cap"):
        g.register_subscription("sub-a", "anthropic", allowance=3)
    with pytest.raises(ValueError, match="exceeds the governor cap"):
        g.register_subscription("sub-a", "anthropic", allowance=99)
...
def test_governor_default_allowance_is_one_never_raised_on_inference() -> None:
    ...
    g = SubscriptionGovernor()
    g.register_subscription("sub-a", "anthropic")  # no allowance arg -> default 1
    g.acquire("sub-a", "n1")
    with pytest.raises(SubscriptionLimitExceeded, match="one terminal per subscription"):
        g.acquire("sub-a", "n2")
...
def test_governor_release_before_acquire_at_allowance_two() -> None:
    ...
    g = SubscriptionGovernor()
    g.register_subscription("sub-a", "anthropic", allowance=2)
...
def test_governor_reregistration_narrowing_binds_no_eviction() -> None:
    ...
    g.register_subscription("sub-a", "anthropic", allowance=1)
...
def test_governor_status_exposes_n_of_allowance_for_status_bar() -> None:
    ...
    g.acquire("sub-a", "n1")
    st = g.status()["sub-a"]
    assert st["in_use"] == 1 and st["allowance"] == 2 and st["provider"] == "anthropic"
