...
def test_ix3_succession_handoff_owned_by_succession_code(server) -> None:
    ...
    gov = SubscriptionGovernor(); gov.register_subscription("sub-anthropic-01", "anthropic")
    gov.acquire("sub-anthropic-01", "conductor-A")
    # acquiring the successor WITHOUT the handoff (predecessor still holding) must fail -> proves
    # the release-first ordering the handoff enforces actually matters
    with pytest.raises(Exception):
        gov.acquire("sub-anthropic-01", "conductor-B")
    order = SuccessionManager(_client(server, "c", "conductor")).perform_handoff(
        gov, "sub-anthropic-01", "conductor-A", "conductor-B")
