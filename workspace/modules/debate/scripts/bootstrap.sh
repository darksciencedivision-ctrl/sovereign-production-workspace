...
if [ "$SKIP_OLLAMA" -eq 1 ]; then
    ...
    CHECK_OUT=$("$VENV_PYTHON" - "$CONFIG_PATH" <<'PYEOF' 2>&1 || true
...
try:
    ...
    print("BADCONFIG|" + str(exc).replace("|", " ")); raise SystemExit(0)
...
PORT=$("$VENV_PYTHON" -c "import json,sys;print(json.load(open(sys.argv[1],encoding='utf-8')).get('port',8700))" "$CONFIG_PATH" 2>/dev/null || echo 8700)
