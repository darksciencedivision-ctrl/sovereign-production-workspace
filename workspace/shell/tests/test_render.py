...
class TestRenderProofs(unittest.TestCase):
    ...
    def test_header_links_resolve(self):
        docs = [
            ...
            ("theme-baseline", os.path.join(WORKSPACE, "docs", "THEME-BASELINE.md")),
        ...
        try:
            for name, path in docs:
                ...
                lines.append("/doc/{:<14} -> 200 {} nosniff; first line matches disk: {!r}"
                             .format(name, ctype, first_body[:60]))
    ...
    def test_light_theme_contrast(self):
        ...
        light_tokens = _tokens(light_block)
        base_rules = css[:light_m.start()] + css[light_m.end():]

        # Which tokens does CSS place in a TEXT position on badges?
        # base rule  .badge-ok { ... color: var(--ok) ... }  -> text-used wherever no override
        text_used = {"dark": set(), "light": set()}
        for state in ("ok", "warning", "danger"):
            if re.search(r"\.badge-{0}\b[^{{}}]*\{{[^}}]*color:\s*var\(--{0}\)".format(state),
                         base_rules, re.S):
                text_used["dark"].add(state)
                text_used["light"].add(state)
        # light-block override demoting badge text to --text promotes states to glyph-only
        for m in re.finditer(r"([^{}]+)\{([^{}]*)\}", light_block):
            sel, body = m.group(1), m.group(2)
            if ".badge-" in sel and re.search(r"color:\s*var\(--text\)", body):
                for state in ("ok", "warning", "danger"):
                    if ".badge-" + state in sel:
                        text_used["light"].discard(state)
        ...
        lines = [header("H-16 - WCAG contrast of the six C-2 token pairs, per theme; "
                        "text pairings asserted >= 4.5:1, glyph pairings recorded vs 3:1")]
        failures = []
        glyph_lines = []
        for theme, overrides in (("dark", None), ("light", light_tokens)):
            # effective tokens: :root values, plus the light-block overrides when present
            ...
            for a, b in pairs:
                ...
                vb = eff.get("--" + b)
                r = _ratio(va, vb)
                if a in text_used[theme] or a in ("text", "text-muted"):
                    ok_flag = r >= 4.5
                    ...
                    g_ok = r >= 3.0
                    glyph_lines.append(
                        "{:<5} --{:<11} on --{:<5} = {:>6.2f}:1  GLYPH {}".format(
                            theme, a, b, r, "(>=3:1)" if g_ok else "(<3:1 - decorative dot "
                            "redundant with its text label; see risk note)"))

        lines.extend(glyph_lines)
        lines.append("")
        lines.append("asserted: text-position pairings >= 4.5:1 in both themes -> {}".format(
